...
public class Purify {
	public static void main(String[] args) {
		...
		String fileName = "/Users/guangyi/Desktop/toolkit/tomcat_log.txt";
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				Pattern pattern = Pattern.compile("\\[(.+?):(.+)");// regression expression . 可以指代任何符号 +表示+前面的那个符号代表的字符可以出现1次到无数次
				...
				db.getCollection("logs")
						.insertOne(new Document().append("ip", ip).append("date", matcher.group(1))
								...
								.append("url", url).append("status", status));//group(0)对应整个正则表达式
			}
